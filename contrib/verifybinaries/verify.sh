...
###   Bwbcoincore.org and Bwbcoin.org and compares them.
...
WORKINGDIR="/tmp/Bwbcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://Bwbcoincore.org"
HOST2="https://Bwbcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="Bwbcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'Bwbcoin-',
...
if [ $? -ne 0 ]; then
   echo "Bwbcoin.org failed to provide signature file, but Bwbcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "Bwbcoin.org and Bwbcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bwbcoin Core binary release signing key installed?"
   fi
