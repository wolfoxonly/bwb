...
// Copyright (c) 2011-2015 The Bwbcoin developers
...
const string strMessageMagic = "Bwbcoin Signed Message:\n";
...
{
    return 0.00001 *COIN;/////POW先改成奖励0.1<coingo.vip>
...
int64 GetProofOfStakeReward(int64 nCoinAge,int64 balance)//<coingo.vip>
{
    ...
    int year =  std::ceil(pindexBest->nHeight / (24 * 60 / 10 * 365));//coingo.vip利率衰减部分
    printf("year ===== %d\n", year);

    float rate = 15*pow(0.9,year);//coingo.vip
    printf("rate ===== %f\n", rate);
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // ppcoin: fees are not collected by miners as in Bwbcoin
...
bool LoadBlockIndex()
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsLimit =%d,nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Bwbcoin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnProofOfWorkLimit.GetCompact(),bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
...
// ~uint256(0) >> 32
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The Times 13/2/2018 BWB genesis";// "Matonis 07-AUG-2012 Parallel Currencies And The Roadmap To Monetary Freedom";
        CTransaction txNew;
        txNew.nTime = 1518487585;//1516498115;//1345083810;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1518487585;// 1345084287;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
      //  block.nNonce   = 2179302059u;
        block.nNonce = ByteReverse(4904202);//<coingo.vip>
        if (fTestNet)
        {
...
/////////////////////////////////coingo.vip11111111111111111111111
 ...
 printf("block.nNonce 11111111111111 coingo.vip:%d \n" ,block.nNonce);
        ...
        assert(block.hashMerkleRoot == uint256("228b225adf97d0e99d6757daa6a45bb7122e3bc971caab0ed94fca958fb4c3b8"));
    ...
    int64 nFees = 0;
    {
        ...
        if (pblock->IsProofOfWork())
        {
          ...
          printf("POW pindexPrev.nHeight===%d\n", pindexPrev->nHeight);
           ...
           {
                pblock->vtx[0].vout[0].nValue = 1650875314.49168 * COIN;//首块产量<coingo.vip>
           }
           else
           {
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("BwbcoinMiner : proof-of-work not meeting target");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BwbcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BwbcoinMiner : ProcessBlock, block not accepted");
    }
...
#ifdef TESTING
void BwbcoinMiner(CWallet *pwallet, bool fProofOfStake, bool fGenerateSingleBlock)
#else
void BwbcoinMiner(CWallet *pwallet, bool fProofOfStake)
#endif
{
    ...
    try { loop {
        ...
        printf("Running BwbcoinMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        if(!fProofOfStake) printf("BwbcoinMiner terminated\n");
        throw;
    }
}

void GenerateBwbcoins(bool fGenerate, CWallet* pwallet)
{
...
#ifdef TESTING
        minerThreads->create_thread(boost::bind(&BwbcoinMiner, pwallet, false, false));
#else
        minerThreads->create_thread(boost::bind(&BwbcoinMiner, pwallet, false));
#endif
}
