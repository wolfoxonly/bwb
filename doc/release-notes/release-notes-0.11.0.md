Bwbcoin Core version 0.11.0 is now available from:

  <https://Bwbcoin.org/bin/Bwbcoin-core-0.11.0/>
...
For example, add the following to `Bwbcoin.conf`:
...
related to the blockchain in the Bwbcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Bwbcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Bwbcoin Core will 
...
For an example script see [Bwbcoin-submittx](https://github.com/laanwj/Bwbcoin-submittx).
...
- #5334 `cf87536` libBwbcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new Bwbcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only BwbcoinAmountField
- #5753 `0f44672` Add Bwbcoin logo to about screen
...
- #5809 `46bfbe7` Add Bwbcoin-cli man page
...
- #5849 `41259ca` contrib/init/Bwbcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Bwbcoin-Qt.app to Bwbcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for Bwbcoin-tx)
