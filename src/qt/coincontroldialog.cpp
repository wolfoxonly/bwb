...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    loop
    {
        ...
        BOOST_FOREACH(const PAIRTYPE(QString, qint64) &payee, CoinControlDialog::payAddresses)
        {
            ...
            scriptPubKey.SetDestination(CBwbcoinAddress(address.toStdString()).Get());
    ...
    int nDisplayUnit = BwbcoinUnits::BTC;
    ...
    l2->setText(BwbcoinUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(BwbcoinUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(BwbcoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? "~" : "") + QString::number(nBytes));                                    // Bytes
    l6->setText(sPriorityLabel);                                             // Priority
    l7->setText((fLowOutput ? (fDust ? tr("DUST") : tr("yes")) : tr("no"))); // Low Output / Dust
    l8->setText(BwbcoinUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    ...
    l7->setToolTip(tr("This label turns red, if any recipient receives an amount smaller than %1.\n\n This means the transaction will be rejected.").arg(BwbcoinUnits::formatWithUnit(nDisplayUnit, CENT)));
    l8->setToolTip(tr("If the change is smaller than %1 it will be added to the fees.").arg(BwbcoinUnits::formatWithUnit(nDisplayUnit, CENT)));
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(PAIRTYPE(QString, vector<COutput>) coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            {
                sAddress = CBwbcoinAddress(outputAddress).ToString().c_str();

                // if listMode or change => show Bwbcoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, BwbcoinUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, BwbcoinUnits::format(nDisplayUnit, nSum));
