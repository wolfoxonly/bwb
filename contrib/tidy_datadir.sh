#!/bin/bash
# Copyright (c) 2013 The Bwbcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Bwbcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Bwbcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Bwbcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Bwbcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Bwbcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Bwbcoin 0.8 datadir."
    ;;
esac
