...
The RPM spec file provided here is for Bwbcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://Bwbcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Bwbcoin git repository](https://github.com/Bwbcoin/Bwbcoin) but are not part of
...
Currently only `Source10` is used, for the example `Bwbcoin.conf` file.
...
in the Bwbcoin git repository. At present only an SVG version of the Bwbcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the Bwbcoin
...
needed when 0.12.1 is released, a proper fix is already in the Bwbcoin git
master branch.
...
idea to build Bwbcoin Core against a static version of that library compiled
...
The `Bwbcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `Bwbcoin-server` package. At this point, most RPM based
...
The `Bwbcoin-server` package should have SELinux support. How to properly do
...
Bwbcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/Bwbcoin/). He has been
packaging Bwbcoin for Fedora at least since 2012.
...
4. I renamed the `Bwbcoin` package that contains the Qt GUI to `Bwbcoin-core` as
that appears to be how the general population refers to it, in contrast to
`Bwbcoin-xt` or `Bwbcoin-classic`. I wanted to make sure the general population
