...
void CWalletTx::RelayWalletTransaction()
{
    ...
    {
        // Important: versions of Bwbcoin before 0.8.6 had a bug that inserted
            ...
            loop
            {
                ...
                if (nChange > 0)
                {
                    ...
                    else if (!GetBoolArg("-avatar")) // ppcoin: not avatar mode; nu: avatar mode enabled by default only on Share wallet to avoid change being sent to hidden address
                    {
                        ...
                        // change transaction isn't always pay-to-Bwbcoin-address
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CBwbcoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CBwbcoinAddress(address).ToString());
}
