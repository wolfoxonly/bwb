...
// Copyright (c) 2011-2017 The Bwbcoin developers
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bwbcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bwbcoin
    // Mac: ~/Library/Application Support/Bwbcoin
    // Unix: ~/.Bwbcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bwbcoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Bwbcoin.conf"));
    ...
    if (mapArgs.count("-conf") == 0 && !boost::filesystem::exists(pathConfigFile)) {
        ...
        else
        {
            ...
            try {
                const char* content = "rpcuser=Bwbcoinrpc\nrpcpassword=Bwbcoinrpcpassword";
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No Bwbcoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override Bwbcoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Bwbcoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Bwbcoin will not work properly.");
...
// Format the subversion field according to BIP 14 spec (https://en.Bwbcoin.it/wiki/BIP_0014)
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "Bwbcoin:" << FormatVersion(BWBCOIN_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
